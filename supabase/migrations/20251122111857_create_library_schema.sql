/*
  ...
  1. New Tables
    - `profiles`
      ...
      - `accessibility_preferences` (jsonb, default empty)
    ...
    - `books`
      ...
      - `pdf_file` (text, optional)
      - `audiobook_file` (text, optional)
    ...
    - `book_requests`
      ...
      - `status` (text, enum: pending/approved/rejected/issued/returned)
  ...
  2. Indexes
    - Books: title, authors (GIN), is_active
    - Book requests: user_id, book_id, status

  3. Security
    - Enable RLS on all tables
    - Profiles: users can view all, update own
    - Books: authenticated users can view active books, librarians+ can insert/update
    - Book requests: users can view own, librarians+ can view all, users can create, librarians+ can update
*/
